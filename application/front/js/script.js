...
function calculate() {
    ...
    for (const operator of operators) {
      if (expression.includes(operator)) {
        ...
        switch (operator) {
          case '+':
            url = 'http://moadservice-api:5000/api/addition';
            break;
          case '-':
            url = 'http://moadservice-api:5000/api/soustraction';
            break;
          case '*':
            url = 'http://moadservice-api:5000/api/multiplication';
            break;
          case '/':
            if (b === 0) {
            ...
            url = 'http://moadservice-api:5000/api/division';
            break;
          default:
  ...
  function getResult() {
    const url = "http://moadservice-api:5000/api/result/" + oper_identifiant;
    fetch(url, {
      method: 'GET',
      headers: { 'Content-Type': 'application/json' },
    })
